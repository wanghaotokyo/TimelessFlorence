...
Windows 11 可选 Kokoro 中文本地神经语音实验版。首次准备会把约 170 MB 模型下载到浏览器缓存；之后使用 Edge 或 Chrome 的 WebAssembly 后台兼容模式在本机生成语音，可断网工作，不需要 TTS Key，也不按次数收费。Windows 系统声音继续作为轻量备用。
