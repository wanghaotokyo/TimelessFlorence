...
export function ArtImage({guide,row,className=''}:{guide:Guide;row?:LocalGuide;className?:string}){
 ...
 useEffect(()=>{setFailed(false);if(row?.offline&&row.imageBlob){const blob=URL.createObjectURL(row.imageBlob);setUrl(blob);return()=>URL.revokeObjectURL(blob);}setUrl(imageUrl(guide));},[guide.id,guide.image,row?.imageBlob,row?.offline]);
